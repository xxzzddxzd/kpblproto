...
def _execute_pingu(account_name, args, **kw):
    from .da_manager import DAManager
    showres = kw.get('showres', 0)
    delay = kw.get('delay', 0)
    ac = kw.get('ac_manager')
    gacha_rounds = int(args[0]) if len(args) > 0 else 4
    buy_times = int(args[1]) if len(args) > 1 else 2
    dm = DAManager(account_name, showres=showres, delay=delay, ac_manager=ac)
...
def _execute_grc(account_name, args, **kw):
    ...
    delay = kw.get('delay', 0)
    ac = kw.get('ac_manager')
    return TradeManager(account_name, showres=showres, delay=delay, ac_manager=ac).run_grc(max_refresh=max_refresh)
