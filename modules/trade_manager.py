...
class TradeManager:
    ...
    GRC_TICKET_ITEM_ID = 133
    GRC_TICKET_SHOP_ID = 22100
    GRC_MIN_TICKET_COUNT = 10
    GRC_UR_RARITY = 5
    GRC_SLOT_COUNT = 2
    ...
    def _parse_grc_reward_response(self, res):
        if not res or len(res) <= 6:
            return None
        resp = kpbl_pb2.grc_personal_boat_reward_response()
        resp.ParseFromString(res[6:])
        return resp
    ...
    def _grc_raw_request(self, ads, header, slot_index=None):
        config = {"ads": ads, "times": 1, "hexstringheader": header}
        if slot_index is not None:
            config["request_body_i2"] = int(slot_index)
    ...
    def _grc_items_text(self, cabin):
        return self._grc_item_pairs_text(self._grc_item_pairs(cabin))

    def _grc_item_pairs_text(self, pairs):
        parts = []
    ...
    def _grc_is_ur(self, cabin):
        return bool(cabin and cabin.rarity >= self.GRC_UR_RARITY)

    def ensure_grc_tickets(self, minimum=None):
        """开个人船前确保船票数量达到最低要求。"""
        minimum = minimum or self.GRC_MIN_TICKET_COUNT
        _, count = self.ac_manager.getItemIdByType(self.GRC_TICKET_ITEM_ID)
        count = int(count or 0)
        if count >= minimum:
            print(f"船票充足: {count}/{minimum}")
            return True

        buy_count = minimum - count
        print(f"船票不足: {count}/{minimum}，购买{buy_count}张")
        req = {
            "ads": f"船票购买({buy_count})",
            "times": 1,
            "hexstringheader": "6532",
            "request_body_i2": 2,
            "request_body_i3": self.GRC_TICKET_SHOP_ID,
            "request_body_i4": buy_count,
        }
        res = self.ac_manager.do_common_request(self.account_name, req, showres=self.showres)
        status = self.ac_manager.get_status_code(res) if res else None
        ok = bool(res and (len(res) > 20 or status == 0))
        print(f"船票购买{'成功' if ok else '失败'}: status={status}, len={len(res) if res else 0}")
        return ok
    ...
    def claim_previous_grc_reward(self, slot_index, cabin=None):
        ...
        if not res:
            print(f"  舱{slot_index} 到港奖励领取失败: 无响应")
            return False, []

        body = res[6:] if len(res) > 6 else b""
        if not body.startswith(b"\x12"):
            try:
                status = self.ac_manager.get_status_code(res)
            except Exception:
                status = None
            print(f"  舱{slot_index} 到港奖励未领取: status={status}, len={len(res)}")
            return False, []

        reward_resp = self._parse_grc_reward_response(res)
        rewards = []
        if reward_resp:
            rewards = [(item.itemid, item.itemcount) for item in reward_resp.result.rewards]
        print(f"  舱{slot_index} 到港奖励领取成功: {self._grc_item_pairs_text(rewards)}")
        return True, rewards
