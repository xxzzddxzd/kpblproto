...
"""
通用请求序列生成器
从 Charles 导出的 .chlz/.chlsj 文件生成请求列表，输出到该文件所在目录下

用法: python generate_requests.py <chlz_or_chlsj_file_or_dir>
...
def generate_requests(source_path):
    """
    从 chlz/chlsj 文件或目录生成请求列表

    Args:
        source_path: .chlz/.chlsj 文件路径或包含会话文件 / *-req.bin 文件的目录
...
def main():
    if len(sys.argv) < 2:
        print("用法: python generate_requests.py <chlz_or_chlsj_file_or_dir>")
        print("示例: python generate_requests.py nuanchunhuodong/Untitled.chlz")
        print("      python generate_requests.py chlz/0601.chlsj")
